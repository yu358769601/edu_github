...
export enum ControlNPCMoveType{
    男的 = 0,
    女的 = 1,
    垃圾桶 = 2,
    路障 = 3,
...
@ccclass
export default class ControlNPCMove extends BaseComponent {
    ...
    gotoRun(npcType,npcNum){
        ...
        if (this.sp) {
            UtilsAction.moveToRepeatForeverXY(this.node,Utils.random(3,7),this.ps[0].x,this.ps[0].y,this.ps[1].x,this.ps[1].y,(type)=>{
                if (type == 0) {
